...
function showStudent (student){
    ...
    <tr data-student-id = "${student.id}">
      ...
      <td>${student.phone}</td>
      <td>
        <button class="btn btn-info me-3 text-light" onclick="insertStudent(${student.id})">Edit</button>
...
function resetForm (){
    ...
    registerInputs.forEach(function(input){
        input.classList.remove("is-valid");
        input.classList.remove("is-invalid");
        let errorElement= document.querySelector(`p.alert[data-error-name="${input.name}"]`);
        errorElement.classList.add("d-none");
    })
...
function noData(data){
    tableAlert = document.querySelector("#tableAlert");
    if(data.length == 0){
...
function insertStudent(id){
    ...
    let editStudent = students.find(function(student){
        ...
        myBtn = registerForm.querySelector("button")
